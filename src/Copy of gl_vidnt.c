...
#define MAX_MODE_LIST	600	//  Baker 3.65 - Qrack has this at 600 -> R00k was 30 now support 32bit at all resolutions!
...
cvar_t		vid_conwidth = {"vid_conwidth", ""};
cvar_t		vid_conheight = {"vid_conheight", ""};
cvar_t		_windowed_mouse = {"_windowed_mouse","1", true};
extern	cvar_t	cl_confirmquit; // Baker 3.60

// Baker 3.65 - vid_vsync from Qrack
static qboolean update_vsync = true;
qboolean OnChange_vid_vsync (cvar_t *var, char *string);
cvar_t	vid_vsync				= {"vid_vsync", "0",true};//R00k set to 0 default to reduce DIB errors


// Baker 3.65 - ^^^^ End vid_vsync from Qrack
...
{
    if (CheckExtension("WGL_EXT_swap_control") || CheckExtension("GL_EXT_swap_control"))
	{
		...
		if (wglSwapIntervalEXT && wglGetSwapIntervalEXT)
		{
			...
			{
				Con_Printf("VSYNC Support Found: WGL_EXT_swap_control\n");
...
/*	if ((CheckExtension("GL_ARB_texture_compression")) || (CheckExtension("GL_EXT_texture_compression_s3tc")))
	{
		Con_Printf("Texture Compression Extension Found\n");
		g_canUseTexComp = true;
	}
	else
	{
		g_canUseTexComp = true;
		Con_Printf("Texture Compression not supported\n");
	} */  // Baker we aren't using the above
}
// Baker 3.65 - End vid_vsync from Qrack
...
void GL_EndRendering (void)
{
	if (!scr_skipupdate || block_drawing)
	{
		...
		{
			if (wglSwapIntervalEXT && update_vsync && vid_vsync.string[0]) //Baker 3.65 - vid_vsync
			wglSwapIntervalEXT(vid_vsync.value ? 1 : 0);               //Baker 3.65 - vid_vsync
			update_vsync = false;										   //Baker 3.65 - vid_vsync
		}
		SwapBuffers(maindc);
	}
...
//BOOL	gammaworks;  Baker 3.60 - JPG gamma fix?
...
void	VID_Shutdown (void)
{
    ...
    if (vid_initialized) {  // Baker 3.60 - Gamma fix Tyrann?
	if (VID_SetGammaRamp)
	...
	if (fActive)
	{
		if (modestate == MS_FULLDIB)
		{
			...
			if (vid_canalttab && vid_wassuspended) {
				...
				// Baker 3.60 - Fix for alt-tab bug in NVidia drivers  from JoeQuake
...
Check_Gamma(unsigned char *pal)
{
	...
	if ((i = COM_CheckParm("-gamma")) == 0) {
	    ...
	    vid_gamma = 0.7;	// Baker 3.60 - didn't implement Tyrann 1.0 default gamma, if someone wants that they can use -gamma 1.0 parameter
	}
	} else
...
/*
===================
VID_FindMode - Baker 3.65 - cycle thru available video modes to locate closest match
===================
*/

int		VID_FindMode (int width, int height, int bpp)
{
	int		bestmatch = -1;
	float	aspectratio, bestaspectdiff, testaspect, aspectdiff;

	// Search for exact width/height match
	for 
	if width and height
		return;
		
// Failed: search for exact width match

for
	if width
		testaspect = width/height;
		aspectdiff = abs(testaspect-aspectratio);
		if closestaspectdiff=-1 || aspectdiff < closestaspectdiff)
		{
			bestmatch = j;
...
void	VID_Init (unsigned char *palette)
{
	...
	int		realwidth, realheight, testheight; // Baker 3.65 - realwidth/height are the initial widths, testheight is for resolution guessing
	int     screenwidth, screenheight; // Baker 3.65 - really better named as the desktop height
	qboolean	widthprovided = false, heightprovided = false; // Baker 3.65 - if user provided width or height, use it no matter what and do not change it
	char	errorinfo[100];
	float	aspect, reverseaspect;//, test43, test85, test169;
	byte	*ptmp;
	...
	screenwidth = GetSystemMetrics (SM_CXSCREEN);  // Baker 3.65 - reworked width/height calcs/method
	screenheight = GetSystemMetrics (SM_CYSCREEN);
	aspect = reverseaspect = ((float)screenwidth / (float)screenheight); // * (320.0 / 240.0);
	reverseaspect = ((float)screenheight / (float)screenwidth); // * (320.0 / 240.0);
	...
	else
	{
		...
		else
		{
			if (COM_CheckParm("-current"))
			{
				width = screenwidth;
				height = screenheight;
				modelist[MODE_FULLSCREEN_DEFAULT].width = width;
				modelist[MODE_FULLSCREEN_DEFAULT].height = height;
			...
			else
			{
				...
				{
					// Baker 3.65 - We were defaulting to 640 ... hmmmm
					// but this doesn't seem appropriate to me
					// we want a lower resolution default for speed
					// but we need a video mode that makes sense and is likely to be
					// supported for each
					width = 640;
					widthprovided = false;
				}
				...
				if (COM_CheckParm("-height"))
				{
					...
					//Con_Printf("debug dashheight %d %d %d",width, height, bpp);
					heightprovided = true;
				}
				else
				{
					// Baker 3.65 -- determine the aspect ratio and attempt to guess the best height
					// Options 4:3, 8:5 and 16:9
					//aspect = ((float)screenwidth / (float)screenheight); // * (320.0 / 240.0);
					//Con_Printf("debug screen %d %d aspect %4.2f\n ", screenwidth, screenheight, aspect);
					heightprovided = false;

					// Baker 3.65 -- let's check and see what the theoretical height would be
					// and try to guess if it would be supported

					if (widthprovided)
					{
						// Width was provided but height was not, find appropriate aspect ratio for height
						if (aspect = 3.0/2.0)  
							testheight= (int)((2.0/3.0) * (float)width);  // 3:2 	
						else if (aspect = 8.0/5.0)  
							testheight= (int)((5.0/8.0) * (float)width);  // 8:5 aspect ratio assumed
						else if (aspect = 16.0/9.0) 
							testheight= (int)((9.0/16.0) * (float)width); // 16/9
						else if (aspect = 5.0/4.0) 
							testheight= (int)((4.0/5.0) * (float)width); // 5:4 
						else												
							testheight= (int)((3.0/4.0) * (float)width); // 4:3 or something else

						//Con_Printf("debug 169 %d %d %d\n",width, height, bpp);

						if (testheight < 400 )
							height = 480; // Baker 3.65 - Let's hope it is 640 width and go for the default for now
						else
							height = testheight;
					}
					else
					{
						// Baker 3.65 -- Neither width nor height were provided, we are free to change
						// the width if the default width doesn't seem right
						testheight = (int)((float)width * reverseaspect);
						if (testheight < 400 )
						{
							// Seems like such a video mode would not likely be supported, let's take the easy
							// way out and just use the current width for now
							width = screenwidth;
							height = screenheight;
						}
						else
			...
			// if they want to force it, add the specified mode to the list
				...
				//Con_Printf("debug: %d %d %d",width, height, bpp);

				do
				{
					if (height>=0) // (COM_CheckParm("-height"))
					{
				...
				if (!vid_default)
				{
					// Baker 3.66 - Better video mode error message
					sprintf (errorinfo, "Specified video mode not available\n\nResolution (%dx%d %d bpp testheight %d %4.2f)", width, height, bpp, testheight, aspect);
					Sys_Error (errorinfo);
	...
	{
		if (width >= 1024)
			vid.conwidth = width / 2; // Baker 3.65 - Make console characters readable so use 50% on 1024 and greater resolutions
		else
			vid.conwidth = width; //Baker 3.65 - make it the actual width, like Qrack does - R00k:was 640
	}
	...
	//vid.aspect = ((float)realheight / (float)realwidth) * (320.0 / 240.0); // Baker 3.65 - formula from Qrack for aspect ratio
	//vid.conheight = vid.conwidth*3 / 4; <--- old way

	/*
	// pick a conheight that matches the correct aspect
	if (vid.aspect < 1)
		vid.conheight = vid.conwidth * 0.625;//R00k added for widescreen support
	else
		vid.conheight = vid.conwidth * 0.750; */  // Baker we are not using this
	...
	GL_Init ();
	GL_Init_Win(); // Baker 3.65 - Qrack vid_vsync
