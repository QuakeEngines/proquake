...
void Sbar_SoloScoreboard (void)
{
	...
	if ((!cls.demoplayback) && cl.gametype != GAME_DEATHMATCH) // Baker 3.75 - do not display skill if demo is playing, because we don't know!
	{
		...
		{
		case 0:
			sprintf (str, "skill: Â·Û˘", current_skill);
			break;
		case 1:
			sprintf (str, "skill: ÓÔÚÌ·Ï", current_skill);
			break;
		case 2:
			sprintf (str, "skill: Ë·Ú‰", current_skill);
			break;
		case 3:
			sprintf (str, "skill: ÓÈÁËÙÌ·ÚÂ", current_skill);
			break;
		}
...
void Sbar_Draw (void)
{
	...
	if (sb_updates >= vid.numpages && !gl_clear.value) //Baker 3.60 - gl_clear fix with sbar
		return;
#else
	if (sb_updates >= vid.numpages) //Baker 3.60 - gl_clear fix with sbar
		return;
#endif
